package dk.itu.thesis.impl.brown;
...
public class LockFreeKSTRQ<E extends Comparable<? super E>, V> {
    ...
    /**
     *
     * DEBUG CODE (FOR TESTBED INSTRUMENTATION)
     *
     */

    private int sumDepths(Node node, int depth) {
        if (node == null) return 0;
        if (node.c == null) return depth;
        int result = depth;
        for (int i=0;i<node.kcount+1;i++) {
            result += sumDepths((Node) node.c.get(i), depth+1);
        }
        return result;
    }
    ...
    public final double averageLeafDegree() {
        return ((double) totalLeafDegree((Node) root.c.get(0).c.get(0))) / totalLeafCount((Node) root.c.get(0).c.get(0));
    }
    private int totalLeafDegree(Node node) {
        if (node == null) return 0;
        if (node.c == null) return node.kcount;
        int sum = 0;
        for (int i=0;i<node.c.length();++i) {
            sum += totalLeafDegree((Node) node.c.get(i));
        }
        return sum;
    }
    private int totalLeafCount(Node node) {
        if (node == null) return 0;
        if (node.c == null) return 1;
        int sum = 0;
        for (int i=0;i<node.c.length();++i) {
            sum += totalLeafCount((Node) node.c.get(i));
        }
        return sum;
    }
