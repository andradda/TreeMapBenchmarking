package dk.itu.thesis.impl.vcas;

/*
Concurrent, Non-blocking Chromatic Tree with Constant Time Snapshotting and Batched Leaves

This is an implementation of the VcasCT-64 algorithm described in the paper:
    "Constant-Time Snapshots with Applications to Concurrent Data Structures"
    Yuanhao Wei, Naama Ben-David, Guy E. Blelloch, Panagiota Fatourou, Eric Ruppert, Yihan Sun
    PPoPP 2021

This data structure supports linearizable get(), containsKey(), putIfAbsent(), remove(), 
rangeScan(), findIf(), multiSearch() and successor(). All operations are lock-free and 
rangeScan(), findIf(), multiSearch() and successor() are also wait-free.

Copyright (C) 2021 Yuanhao Wei

This implementation is based on the following non-blocking Chromatic Tree implementation by Trevor Brown:
...
import dk.itu.thesis.impl.vcas.Camera;
import dk.itu.thesis.impl.vcas.support.Element;
import dk.itu.thesis.impl.vcas.support.Epoch;
import dk.itu.thesis.impl.vcas.support.Reclaimable;
import dk.itu.thesis.impl.vcas.support.NodeStats;
...
public class VcasBatchChromaticMapGC<K extends Comparable<? super K>,V> {
    ...
    public VcasBatchChromaticMapGC() {
        this(16, DEFAULT_d);
    }

    public VcasBatchChromaticMapGC(final int BATCHING_DEGREE) {
        this(BATCHING_DEGREE, DEFAULT_d); 
    }
    public VcasBatchChromaticMapGC(final int BATCHING_DEGREE, final int allowedViolationsPerPath) {
    ...
    public final NodeStats countNodes(final boolean countVersionList) {
        ...
        return countNodes(root, countVersionList, visited);
    }

    public final NodeStats countNodes(final Node node, final boolean countVersionList, HashSet<Node> visited) {
        if(node == null) return new NodeStats(0, 0);
        if(visited.contains(node)) return new NodeStats(0, 0);
        visited.add(node);
        NodeStats stats = new NodeStats(0, 0);
        if(countVersionList)
            stats.add(countNodes(node.nextv, countVersionList, visited));
        if(node instanceof LeafNode)
            stats.externalNodes++;
        else {
            stats.internalNodes++;
            stats.add(countNodes(((InternalNode)node).getLeft(), countVersionList, visited));
            stats.add(countNodes(((InternalNode)node).getRight(), countVersionList, visited));
        }
        return stats;
    }   
    ...
    public final V putIfAbsent(final K key, final V value) {
        ...
        while (true) {
            while (op == null) {
                ...
                if(ret != null) {
                    //epoch.unannounce();
                    return ret; // if we find the key in the tree already
                }   
        ...
        while (true) {
            while (op == null) {
                ...
                if (n instanceof InternalNode) {
                    ...
                    while (n instanceof InternalNode) {
                        InternalNode nn = (InternalNode) n;
                        if (d > 0 && (nn.weight > 1 || nn.weight == 0 && p.weight == 0)) {
                            ++count;
                        }
                        p = nn;
                        n = (key.compareTo((K) nn.key) < 0) ? nn.getLeft() : nn.getRight();
                ...
                if (ret != null) {
                    op = createReplaceExistingOp(p, l, key, value);
                    structuralChange = false;
                } else if (l.getSize() == BATCHING_DEGREE) {
                    op = createInsertOp(p, l, key, value);
                    structuralChange = true;
                } else {
                    op = createInsertReplaceOp(p, l, key, value);
    ...
    //                     p = l;
    //                     l = (key.compareTo((K) l.key) < 0) ? l.left : l.getRight();
    //                 }
    //             }
    ...
    private Operation createReplaceExistingOp(final InternalNode p, final LeafNode l, final K key, final V value) {
        final Operation[] ops = new Operation[]{null};
        final Node[] nodes = new Node[]{null, l};
        ...
        final Node subtree = l.replace(key, value);
        return new Operation(nodes, ops, subtree);
    }
    ...
    public static final class LeafNode extends Node {
        ...
        */
        ...
        private final LeafNode replace(final Comparable key, final Object value) {
            int size = getSize();
            LeafNode newNode = new LeafNode(size, weight);
    ...
    */
    ...
    private final int successorsHelper(final Node node, final long ts, final K key, final int numSuccessors, Element<K,V>[] elements, final int index) {
        ...
        if (node instanceof LeafNode) {    // node is a leaf
            ...
            if (n.key != null && key.compareTo((K) n.key) >= 0)
                return successorsHelper(n.getRight(ts), ts, key, numSuccessors, elements, index);  // traverse its right subtree
            else {
                int newIndex = successorsHelper(n.getLeft(ts), ts, key, numSuccessors, elements, index);  // traverse its right subtree
                if(newIndex < numSuccessors)
                    newIndex = successorsHelper(n.getRight(ts), ts, key, numSuccessors, elements, newIndex);   // traverse its left subtree
                return newIndex;
            }
        }
    }
    ...
    */
    public final Element<K,V>[] successors(final K key, int numSuccessors) {
        ...
        // Object[] values = new Object[numSuccessors];
        long ts = Camera.takeSnapshot();
        int size = successorsHelper(root, ts, key, numSuccessors, elements, 0);
        epoch.unannounce();
    ...
    */
    public final Object[] multiSearch(final K[] keys) {
        ...
        for(int i = 0; i < numKeys; i++)
            returnArray[i] = get(keys[i], ts);
        epoch.unannounce();
        return returnArray;
    }

    private Element<K,V> findIfHelper(Node node, long ts, K a, K b, Predicate<Element<K,V>> p) {
        if(node == null) return null;
        if(node instanceof LeafNode) {
            LeafNode n = (LeafNode) node;
            ...
            if (n.key != null && a.compareTo((K) n.key) >= 0)           // node's key is below the lower limit of [a,b]
                return findIfHelper(n.getRight(ts), ts, a, b, p);  // traverse its right subtree
            else if (n.key == null || b.compareTo((K) n.key) < 0)       // node's key is above the upper limit of [a,b]
                return findIfHelper(n.getLeft(ts), ts, a, b, p);   // traverse its left subtree
            else {
                // node is in [a,b] - traverse both of its subtrees
                Element<K,V> e = findIfHelper(n.getLeft(ts), ts, a, b, p);
                if(e != null) return e;
                return findIfHelper(n.getRight(ts), ts, a, b, p);
            }
        }
    }
    ...
    */
    public Element<K,V> findIf(K lo, K hi, Predicate<Element<K,V>> p) {
        epoch.announce();
        long ts = Camera.takeSnapshot();
        Element<K,V> retValue = findIfHelper(root, ts, lo, hi, p);
        epoch.unannounce();
        return retValue;
    }
